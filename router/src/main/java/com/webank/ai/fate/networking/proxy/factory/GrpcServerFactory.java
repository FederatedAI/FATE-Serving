...
public class GrpcServerFactory {
    ...
    public Server createServer(ServerConf serverConf) {
        ...
        String  routerFile=System.getProperty("router_file");

        if(StringUtils.isNotEmpty(routerFile)) {

            fdnRouter.setRouteTable(routerFile);
        }else{
        ...
        if (StringUtils.isBlank(serverConf.getIp())) {
            ...
            serverBuilder = FateServerBuilder.forNettyServerBuilderAddress(addr);
    ...
    public Server createServer(String confPath) throws IOException {
        ...
        serverConf.setProperties(properties);
