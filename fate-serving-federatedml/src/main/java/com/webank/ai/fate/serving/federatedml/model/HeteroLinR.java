...
public abstract class HeteroLinR extends BaseComponent {
    ...
    public int initModel(byte[] protoMeta, byte[] protoParam) {
        logger.info("start init HeteroLinR class");
        try {
        ...
        logger.info("Finish init HeteroLinR class, model weight is {}", this.weight);
        return OK;
    }
    ...
    Map<String, Double> forwardParallel(List<Map<String, Object>> inputDatas) {
        Map<String, Object> inputData = inputDatas.get(0);
        Map<String, Double> ret = new HashMap<>(8);
        double modelWeightHitRate;
        double inputDataHitRate;
        ...
        if (result != null) {
            try {
                ...
                score = lrTaskResult.score + this.intercept;
                ret.put(Dict.SCORE, score);
    ...
    public class LinRTaskResult {
        double score;
        int modelWeightHitCount;
        int inputDataHitCount;
